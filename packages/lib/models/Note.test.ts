...
// -----------------------------------------------------------------
// ----------------- INÍCIO TESTES MC/DC (ATIVIDADE) -----------------
// Cole este bloco no final de packages/lib/models/Note.test.ts
// -----------------------------------------------------------------

describe('Note.save() MC/DC Tests (Atividade)', () => {

    // O 'beforeEach' e 'afterEach' do arquivo principal vão
    // cuidar de limpar o banco de dados para nós.
    // Vamos precisar de um folder (caderno) para criar notas.
    let folder1: any = null;

    beforeEach(async () => {
        // Cria um caderno padrão onde as notas podem ser salvas
        folder1 = await Folder.save({ title: 'Test Folder' });
    });

    // CT1: Nota Nova, SEM 'order' (T.V. Linha 1)
    // Deve definir o 'order' automaticamente.
    test('CT1: (MC/DC) Deve definir um "order" padrão para uma nota nova que não o possui', async () => {
        // PREPARAÇÃO (C1=V, C2=V)
        const isNew = true; // Implícito ao chamar .save() sem ID
        const noteData = {
            title: 'CT1 - Nota Nova sem order',
            parent_id: folder1.id,
            // 'order' está ausente (C2 = V)
        };

        // EXECUÇÃO
        const savedNote = await Note.save(noteData, { isNew: isNew });

        // VERIFICAÇÃO (Saída Esperada)
        // O resultado da decisão C1 && C2 deve ser V (Verdadeiro)
        // Portanto, o 'order' deve ser definido como um número (timestamp)
        expect(savedNote.order).toBeDefined();
        expect(typeof savedNote.order).toBe('number');
        expect(savedNote.order).toBeGreaterThan(0);
    });

    // CT2: Nota Nova, COM 'order' (T.V. Linha 2)
    // Deve respeitar o 'order' existente.
    test('CT2: (MC/DC) Deve manter o "order" existente ao salvar uma nota nova que já o possui', async () => {
        // PREPARAÇÃO (C1=V, C2=F)
        const isNew = true;
        const definedOrder = 12345; // Ordem específica
        const noteData = {
            title: 'CT2 - Nota Nova com order',
            parent_id: folder1.id,
            order: definedOrder, // 'order' está presente (C2 = F)
        };
        ...
        // VERIFICAÇÃO (Saída Esperada)
        // O resultado da decisão C1 && C2 deve ser F (Falso)
        // Portanto, o 'order' original (12345) deve ser mantido
        expect(savedNote.order).toBe(definedOrder);
    });

    // CT3: Nota Antiga (Edição), SEM 'order' (T.V. Linha 3)
    // Não deve definir um novo 'order'.
    test('CT3: (MC/DC) Não deve definir um novo "order" ao editar uma nota (isNew=false)', async () => {
        // PREPARAÇÃO (Setup)
        // 1. Criar uma nota inicial (sem 'order' definido)
        //    Nota: O CT1 já prova que o .save() inicial define um 'order'.
        //    Vamos criar uma nota e depois "simular" uma edição onde 'order' não é passado.
        const initialNote = await Note.save({
            title: 'CT3 - Nota Original',
            parent_id: folder1.id,
        });
        const originalOrder = initialNote.order;

        // PREPARAÇÃO (C1=F, C2=V)
        const isNew = false; // Estamos editando
        const editData = {
            id: initialNote.id,
            title: 'CT3 - Título Editado',
            // 'order' está ausente na *atualização* (C2 = V)
        };

        // EXECUÇÃO
        const updatedNote = await Note.save(editData, { isNew: isNew });

        // VERIFICAÇÃO (Saída Esperada)
        // O resultado da decisão C1 && C2 deve ser F (Falso)
        // Portanto, o 'order' não deve ser alterado para um novo timestamp.
        // Deve permanecer o 'order' original.
   // ... (fim do CT3) ...
        expect(updatedNote.order).toBe(originalOrder);
    });

}); // <--- Este }); fecha o bloco MC/DC.

// ----------------- FIM TESTES MC/DC (ATIVIDADE) -----------------


}); // <--- ADICIONE ESTA LINHA! Esta é a linha que fecha o arquivo e corrige o erro.